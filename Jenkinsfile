pipeline {
    ...
    stages {
        stage("Collecting Dependencies & Building") {
            steps {
                sh "echo Please wait.."
                sh "sudo npm i && sudo npm run build"
            }
        }
        stage("Deploy") {
            steps {
                sh "rsync -rav --delete build/* administrator@172.16.0.235:/var/www/html/react-pipeline/arbitrage-bot-tushar-24024195-reactjs-admin"
                sh "echo admin-arbitragetushar.mobiloitte.io"
            }
        }
    }
    post {
        always {
            script {
                ...
                emailext to: 'prem.shankar@indicchain.com,re-suraj@mobiloitte.com,de-anil@mobiloitte.com,team-it@mobiloitte.com',
